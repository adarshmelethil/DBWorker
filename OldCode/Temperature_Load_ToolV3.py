...
def print_info(DS, INTs, loads, temps, irri, HUC, customers, BHUC):
  ...
  if sum([INT[1] for INT in INTs]) != 1.00 and len([INT[1] for INT in INTs]) > 1:
    ...
    if len([INT[1] for INT in INTs]) == 1:
      INTs[0][1] = 1
    table1 = [ ['Interconnection #', 'Split (%)'] ] \
    + [ [INT[0], INT[1]*100] for INT in INTs] \
    + [ ["Total Splits:", sum([INT[1]*100 for INT in INTs])] ]
    print(SingleTable(table1, title="Distribution System {0}".format(DS)).table)

  table2 = [ ['Irri Customers', 'HUC w/ Good FQC', 'HUC w/ Bad FQC', 'Total Customers in DS'] ] \
    + [ [irri, HUC, BHUC, customers] ]
  print(SingleTable(table2, title="Customers in Distribution System").table)

  table3 = [ ['Temperature', 'Load(GJ/d)'] ] \
  + zip(temps, np.round(loads, 2))
  print(SingleTable(table3, title="System Demand WZ:{0}".format(WZ)).table)
...
def check_huc(cl):
  ...
  HUC = [customer
    for customer in cl
      if customer['Rate'] in ['HIGH'] and
         customer['FQC'] in ['0', '1', '20']
         ]
...
def ds_path(db_conn, db_north, db_south, DS):
  ...
  total_customers = len(CLs)+len(irri)+len(b_fqc)
...
If yes, 10 GJ/d will be added to the loads at different temperatures.
""")
    while True:
      if IRRI.lower() == 'y':
        loads += 10*len(irri)
      ...
      if IRRI.lower() == 'n':
        break
      IRRI = raw_input('''
Stop being a Raymond, try again with 'y'for yes and 'n' for no:
''')

  if len(b_fqc) > 0:
    fqc = raw_input("""
Do you want to include the 3-year peak of bad FQC customers in the load calculation? [y/n]
If yes, the three year 24 hour peak demand of HUC with bad FQC will be added to the load at
EACH temperature. Otherwise, these customers will be excluded and can be manually added later.
""")
    while True:
      if fqc.lower() == "y":
...
''')
  ...
  print_info(DS, INTs, loads, temps, len(irri), len(HUC), total_customers, len(b_fqc))
